...
public class BillSearchParams extends PageVO {
    ...
    public <T> QueryWrapper<T> queryWrapper() {
        ...
        if (StringUtils.isNotEmpty(startDate) && StringUtils.isNotEmpty(endDate)) {
            ...
            wrapper.ge("b.create_time", startDate);
        } else if (StringUtils.isNotEmpty(endDate)) {
            wrapper.le("b.create_time", endDate);
        }
        //账单号
        ...
        //店铺名称
        wrapper.eq(StringUtils.isNotEmpty(storeName), "b.store_name", storeName);
        //按卖家查询
        wrapper.eq(StringUtils.equals(Objects.requireNonNull(UserContext.getCurrentUser()).getRole().name(), UserEnums.STORE.name()),
                "b.store_id", UserContext.getCurrentUser().getStoreId());
        if (StringUtils.isNotEmpty(tenantId)) {
            wrapper.eq("b.tenant_id", tenantId);
        }
