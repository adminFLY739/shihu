...
public interface BillMapper extends BaseMapper<Bill> {
     ...
     * @return 结算单分页
     */
    @Select("select b.id,b.sn,b.start_time,b.end_time,b.bill_status,b.store_name,b.bill_price,b.create_time,name from li_bill as b join tenant on b.tenant_id = tenant.id ${ew.customSqlSegment}")
    ...
    @Select("SELECT IFNULL(SUM( final_price ),0) AS orderPrice,IFNULL(SUM( commission_price ),0) AS commissionPrice" +
            ...
            "FROM li_store_flow as sf join li_goods as g on sf.goods_id = g.id ${ew.customSqlSegment}")
    ...
    @Select("SELECT IFNULL(SUM( final_price ),0) AS refundPrice,IFNULL(SUM( commission_price ),0) AS refundCommissionPrice" +
            ...
            ",IFNULL(SUM( final_price ),0) AS billPrice FROM li_store_flow as sf join li_goods as g on sf.goods_id = g.id ${ew.customSqlSegment}")
