...
public class BillServiceImpl extends ServiceImpl<BillMapper, Bill> implements BillService {
    ...
    public void createBill(String storeId, Date startTime, DateTime endTime) {
        ...
        tenantList.forEach(tenant -> {
          //获取结算店铺
          StoreDetailVO store = storeDetailService.getStoreDetailVO(storeId,tenant.getId());
          Bill bill = new Bill();
          ...
          //退款结算信息
          Bill refundBill = this.baseMapper.getRefundBill(new QueryWrapper<Bill>().eq("sf.store_id", storeId).eq("tenant_id",tenant.getId()).eq("flow_type", FlowTypeEnum.REFUND.name()).between("sf.create_time", startTime, endTime));
          //店铺退款金额
           ...
           *  @TODO 入账结算信息
           */
          Bill orderBill = this.baseMapper.getOrderBill(new QueryWrapper<Bill>().eq("sf.store_id", storeId).eq("tenant_id",tenant.getId()).eq("flow_type", FlowTypeEnum.PAY.name()).between("sf.create_time", startTime, endTime));
          //店铺入款结算金额
...
//       this.createBill(storeId, startTime, endTime);
    ...
    public boolean complete(String id) {
        ...
        //判断操作人员为后台管理员
        if (!Objects.requireNonNull(UserContext.getCurrentUser()).getRole().equals(UserEnums.MANAGER)) {
            throw new ServiceException(ResultCode.USER_AUTHORITY_ERROR);
        }
        LambdaUpdateWrapper<Bill> lambdaUpdateWrapper = Wrappers.lambdaUpdate();
        lambdaUpdateWrapper.eq(Bill::getId, id);
