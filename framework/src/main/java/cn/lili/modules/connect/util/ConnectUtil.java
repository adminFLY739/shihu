...
public class ConnectUtil {
    ...
    public AuthRequest getAuthRequest(String type) {
        ...
        switch (authInterface) {
            ...
            case WECHAT_PC: {
                //寻找配置
                Setting setting = settingService.get(SettingEnum.WECHAT_CONNECT.name());//获得数据库中id为WECHAT_CONNECT的setting数据
                //获得seting对象中的seting_values属性，并转成WechatConnectSetting类，这个类是连接微信的配置
                //appId：当前产品的ID
                //appsecret：是加密时的私匙
                WechatConnectSetting wechatConnectSetting = JSONUtil.toBean(setting.getSettingValue(), WechatConnectSetting.class);
