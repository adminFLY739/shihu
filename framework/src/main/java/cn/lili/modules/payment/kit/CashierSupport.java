...
public class CashierSupport {
    ...
    public ResultMessage<Object> payment(PaymentMethodEnum paymentMethodEnum, PaymentClientEnum paymentClientEnum,
                                         ...
                                         PayParam payParam,String appid) {
        ...
        switch (paymentClientEnum) {
            ...
            case MP:
                return payment.mpPay(request, payParam,appid);
            default:
                return null;
        }
    }
    ...
    public void notify(PaymentMethodEnum paymentMethodEnum,
                       HttpServletRequest request ,String appid) {

        log.info("支付异步通知：支付类型：{}", paymentMethodEnum.name());
        log.info("appid：{}", appid);
        //获取支付插件
        Payment payment = (Payment) SpringContextUtil.getBean(paymentMethodEnum.getPlugin());
        payment.notify(request,appid);
    }
     ...
     * @return 收银台参数
     */
    public CashierParam cashierParam(PayParam payParam,String appid) {
        for (CashierExecute paramInterface : cashierExecuteList) {
            ...
            cashierParam.setWalletValue(memberWalletService.getMemberWallet(Objects.requireNonNull(UserContext.getCurrentUser()).getId(),tenant.getId()).getMemberWallet());
            OrderSetting orderSetting = JSONUtil.toBean(settingService.getByIdAndTenantId(SettingEnum.ORDER_SETTING.name(),"0").getSettingValue(), OrderSetting.class);
