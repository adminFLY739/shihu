...
public class WechatPlugin implements Payment {
    ...
    public ResultMessage<Object> h5pay(HttpServletRequest request, HttpServletResponse response1, PayParam payParam) {
        ...
        try {
            CashierParam cashierParam = cashierSupport.cashierParam(payParam,"0");
            ...
            WechatPaymentSetting setting = wechatPaymentSetting("1");
    ...
    public ResultMessage<Object> jsApiPay(HttpServletRequest request, PayParam payParam) {
        ...
        try {
            ...
            boolean verifySignature = WxPayKit.verifySignature(response, getPlatformCert(appid));
    ...
    @Override
    public ResultMessage<Object> mpPay(HttpServletRequest request, PayParam payParam,String appid) {

        try {
            Connect connect = connectService.queryConnect(
                    ConnectQueryDTO.builder().userId(UserContext.getCurrentUser().getId()).unionType(ConnectEnum.WECHAT_MP_OPEN_ID.name()).appid(appid).build()
            );
            if (connect == null) {
                return null;
            }
            ...
            CashierParam cashierParam = cashierSupport.cashierParam(payParam,appid);
            ...
            WechatPaymentSetting setting = wechatPaymentSetting(appid);
            UnifiedOrderModel unifiedOrderModel = new UnifiedOrderModel()
                    .setAppid(appid)
                    .setMchid(setting.getMchId())
                    .setDescription(cashierParam.getDetail())
                    ...
                    .setTime_expire(timeExpire)
                    .setAttach(attach)
                    .setNotify_url(notifyUrl(apiProperties.getBuyer(), PaymentMethodEnum.WECHAT)+"/"+appid)
                    .setAmount(new Amount().setTotal(fen))
                    .setPayer(payer);
    ...
    @Override
    public void notify(HttpServletRequest request,String appid) {
        try {
            verifyNotify(request,appid);
        } catch (Exception e) {
            log.error("支付异常", e);
        }
    }
    ...
    public void transfer(MemberWithdrawApply memberWithdrawApply) {
        ...
        try {
            Setting systemSetting = settingService.getByIdAndTenantId(SettingEnum.WECHAT_PAYMENT.name(),memberWithdrawApply.getTenantId());
            WechatPaymentSetting setting = JSONUtil.toBean(systemSetting.getSettingValue(), WechatPaymentSetting.class);
            Connect connect = connectService.queryConnect(
                    ConnectQueryDTO.builder().userId(memberWithdrawApply.getMemberId())
                            .unionType(ConnectEnum.WECHAT_MP_OPEN_ID.name()).appid(setting.getMpAppId()).build()
            );
            //根据自身情况设置AppId,此处我存放的是服务号的APPID，下方的openID需要对应此处的APPID配置
            TransferModel transferModel = new TransferModel()
                    .setAppid(setting.getMpAppId())
     ...
     * @throws Exception
     */
    private void verifyNotify(HttpServletRequest request,String appid) throws Exception {
        ...
        String plainText = WxPayKit.verifyNotify(serialNo, result, signature, nonce, timestamp,
                setting.getApiKey3(), Objects.requireNonNull(getPlatformCert(appid)));
    ...
    public void refund(RefundLog refundLog) {
        ...
        try {
            ...
            Goods goods = goodsService.getById(refundLog.getGoodsId());
            Setting systemSetting = settingService.getByIdAndTenantId(SettingEnum.WECHAT_PAYMENT.name(),goods.getTenantId());
            WechatPaymentSetting setting = JSONUtil.toBean(systemSetting.getSettingValue(), WechatPaymentSetting.class);
            //退款参数准备
            RefundModel refundModel = new RefundModel()
                    ...
                    .setAmount(amount)
                    .setNotify_url(refundNotifyUrl(apiProperties.getBuyer(), PaymentMethodEnum.WECHAT)+"/"+setting.getMpAppId());
    ...
    @Override
    public void refundNotify(HttpServletRequest request,String appid) {
        String timestamp = request.getHeader("Wechatpay-Timestamp");
        String nonce = request.getHeader("Wechatpay-Nonce");
        String serialNo = request.getHeader("Wechatpay-Serial");
        String signature = request.getHeader("Wechatpay-Signature");
        ...
        try { //校验服务器端响应¬
            String plainText = WxPayKit.verifyNotify(serialNo, result, signature, nonce, timestamp,
                    wechatPaymentSetting(appid).getApiKey3(), Objects.requireNonNull(getPlatformCert(appid)));
     ...
     * @return
     */
    private WechatPaymentSetting wechatPaymentSetting(String appid) {
        try {
            Tenant tenant =  tenantAreaService.getTenantByAppId(appid);
            Setting systemSetting = settingService.getByIdAndTenantId(SettingEnum.WECHAT_PAYMENT.name(),tenant.getId());
     ...
     * @return 平台公钥
     */
    private X509Certificate getPlatformCert(String appid) {
        //获取缓存中的平台公钥，如果有则直接返回，否则去微信请求
        String publicCert = cache.getString(CachePrefix.WECHAT_PLAT_FORM_CERT.getPrefix()+appid);
        ...
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                publicCert = getPlatformCertStr(associatedData, nonce, cipherText,appid);
                long second = (PayKit.getCertificate(publicCert).getNotAfter().getTime() - System.currentTimeMillis()) / 1000;
                cache.put(CachePrefix.WECHAT_PLAT_FORM_CERT.getPrefix()+appid, publicCert, second);
            } else {
     ...
     */
    private String getPlatformCertStr(String associatedData, String nonce, String cipherText,String appid) throws GeneralSecurityException {


        AesUtil aesUtil = new AesUtil(wechatPaymentSetting(appid).getApiKey3().getBytes(StandardCharsets.UTF_8));
        //平台证书密文解密
