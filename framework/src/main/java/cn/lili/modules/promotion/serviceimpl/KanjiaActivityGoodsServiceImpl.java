...
public class KanjiaActivityGoodsServiceImpl extends AbstractPromotionsServiceImpl<KanJiaActivityGoodsMapper, KanjiaActivityGoods> implements KanjiaActivityGoodsService {
    ...
    @Override
    public IPage<KanjiaActivityGoodsVO> queryKanjiaActivityGoods(IPage<KanjiaActivityGoodsVO> page, Wrapper<KanjiaActivityGoodsVO> queryWrapper) {
        return baseMapper.queryKanjiaActivityGoods(page, queryWrapper);
    }
    ...
    private void checkParam(KanjiaActivityGoodsDTO kanJiaActivityGoodsDTO, GoodsSku goodsSku) {
        ...
        //校验最高砍价金额
        if (kanJiaActivityGoodsDTO.getHighestPrice() > goodsSku.getPrice()-kanJiaActivityGoodsDTO.getSettlementPrice() || kanJiaActivityGoodsDTO.getHighestPrice() <= 0) {
            throw new ServiceException(ResultCode.KANJIA_GOODS_ACTIVE_HIGHEST_PRICE_ERROR);
        }
        //校验最低砍价金额
        if (kanJiaActivityGoodsDTO.getLowestPrice() > goodsSku.getPrice()-kanJiaActivityGoodsDTO.getSettlementPrice() || kanJiaActivityGoodsDTO.getLowestPrice() <= 0) {
