...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply> implements SeckillApplyService {
    ...
    @Override
    public List<SeckillTimelineVO> getSeckillTimeline(String tenantId) {
        try {
            //秒杀活动缓存key
            return getSeckillTimelineInfo(tenantId);
        } catch (Exception e) {
    ...
    @Override
    public List<SeckillGoodsVO> getSeckillGoods(Integer timeline,String tenantId) {
        try {
            List<SeckillGoodsVO> seckillGoodsVoS = new ArrayList<>();
            //获取
            List<SeckillTimelineVO> seckillTimelineToCache = getSeckillTimelineInfo(tenantId);
    ...
    public IPage<SeckillApply> getSeckillApplyPage(SeckillSearchParams queryParam, PageVO pageVo) {
        IPage<SeckillApply> seckillApplyPage = baseMapper.querySeckillApply(PageUtil.initPage(pageVo), queryParam.queryWrapper());
     ...
     * @return 秒杀活动信息
     */
    private List<SeckillTimelineVO> getSeckillTimelineInfo(String tenantId) {
        ...
        for (Seckill seckill : seckillList) {
            ...
            for (int i = 0; i < hoursSored.length; i++) {
                ...
                if (hoursSoredHour || lastHour) {
                    ...
                    tempTimeline.setSeckillGoodsList(wrapperSeckillGoods(hoursSored[i], seckill.getId(),tenantId));
     ...
     * @return 当时间秒杀活动的商品数据
     */
    private List<SeckillGoodsVO> wrapperSeckillGoods(Integer startTimeline, String seckillId,String tenantId) {
        List<SeckillGoodsVO> seckillGoodsVoS = new ArrayList<>();
//        List<SeckillApply> seckillApplyList = this.list(new LambdaQueryWrapper<SeckillApply>().eq(SeckillApply::getSeckillId, seckillId));
        List<SeckillApply> seckillApplyList = baseMapper.seckillApplyList(seckillId,tenantId);
