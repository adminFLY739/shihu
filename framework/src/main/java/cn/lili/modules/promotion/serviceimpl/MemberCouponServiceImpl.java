...
public class MemberCouponServiceImpl extends ServiceImpl<MemberCouponMapper, MemberCoupon> implements MemberCouponService {
    ...
    @Override
    public void checkCouponLimit(String couponId, String memberId,String tenantId) {
        ...
        LambdaQueryWrapper<MemberCoupon> queryWrapper = new LambdaQueryWrapper<MemberCoupon>()
                ...
                .eq(MemberCoupon::getTenantId, tenantId)
     ...
     */
    @Override
    @CacheEvict(key = "#memberId+#tenantId")
    @Transactional(rollbackFor = Exception.class)
    public void receiveBuyerCoupon(String couponId, String memberId, String memberName,String tenantId) {
        Coupon coupon = couponService.getById(couponId);
        if (coupon != null && !CouponGetEnum.FREE.name().equals(coupon.getGetType())) {
            ...
            this.receiverCoupon(couponId, memberId, memberName, coupon,tenantId);
        }
    ...
    @CacheEvict(key ="#memberId+#tenantId")
    @Transactional(rollbackFor = Exception.class)
    public void receiveCoupon(String couponId, String memberId, String memberName,String tenantId) {
        Coupon coupon = couponService.getById(couponId);
        if (coupon != null) {
            this.receiverCoupon(couponId, memberId, memberName, coupon,tenantId);
        } else {
    ...
    public IPage<MemberCoupon> getMemberCoupons(MemberCouponSearchParams param, PageVO pageVo) {
        ...
        if (page.getRecords() != null && page.getRecords().size() > 0) {
            if (page.getRecords().stream().anyMatch(i -> i.getEndTime().before(new Date()))) {
                this.expireInvalidMemberCoupon(param.getMemberId(),param.getTenantId());
     ...
     */
    @Override
    @Cacheable(key = "#memberId+#tenantId")
    public List<MemberCoupon> getMemberCoupons(String memberId,String tenantId) {
    ...
    @Override
    public long getMemberCouponsNum(String tenantId) {
    ...
    @CacheEvict(key = "#memberId+#tenantId")
    @Transactional(rollbackFor = Exception.class)
    public void used(String memberId, List<String> ids,String tenantId) {
    ...
    public Page<MemberCouponVO> getMemberCouponsPage(Page<MemberCoupon> page, MemberCouponSearchParams param) {
        ...
        queryWrapper.eq(CharSequenceUtil.isNotEmpty(param.getTenantId()), "mc.tenant_id", param.getTenantId());
    ...
    @Override
    @CacheEvict(key = "#memberId+#tenantId")
    public boolean expireInvalidMemberCoupon(String memberId,String tenantId) {
        //将过期优惠券变更为过期状体
        LambdaUpdateWrapper<MemberCoupon> updateWrapper = new LambdaUpdateWrapper<MemberCoupon>()
                ...
                .eq(CharSequenceUtil.isNotEmpty(tenantId), MemberCoupon::getTenantId, tenantId)
    ...
    private void receiverCoupon(String couponId, String memberId, String memberName, Coupon coupon,String tenantId) {
        this.checkCouponLimit(couponId, memberId,tenantId);
        MemberCoupon memberCoupon = new MemberCoupon(coupon);
        memberCoupon.setMemberId(memberId);
        memberCoupon.setMemberName(memberName);
        memberCoupon.setTenantId(tenantId);
