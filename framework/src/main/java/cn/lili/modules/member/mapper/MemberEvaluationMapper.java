...
public interface MemberEvaluationMapper extends BaseMapper<MemberEvaluation> {
     ...
     * @return 会员评价分页
     */
    @Select("select me.* from li_member_evaluation as me join li_goods as m on me.goods_id = m.id  ${ew.customSqlSegment} ")
     ...
     */
    @Select("select me.* from li_member_evaluation as me join li_goods as m on me.goods_id = m.id  ${ew.customSqlSegment} ")
    IPage<MemberEvaluation> managerQuery(IPage<MemberEvaluation> page, @Param(Constants.WRAPPER) Wrapper<MemberEvaluation> queryWrapper);
}
