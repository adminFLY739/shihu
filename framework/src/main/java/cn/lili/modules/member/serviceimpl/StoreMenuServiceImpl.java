...
public class StoreMenuServiceImpl extends ServiceImpl<StoreMenuMapper, StoreMenu> implements StoreMenuService {
    ...
    // @Override
    // public List<StoreMenuVO> tree(String tenantId) {
    //     try {
    //         List<StoreMenu> menus;
    //         if ("0".equals(tenantId)){
    //             menus = getMenuByTenantId();
    //         }
    //         menus = getMenuByTenantId(tenantId);
    //         return tree(menus);
    //     } catch (Exception e) {
    //         log.error("菜单树错误", e);
    //     }
    ...
    // }
    //
    // public List<StoreMenu> getMenuByTenantId(String tenantId) {
    //     try {
    //         QueryWrapper<StoreMenu> queryWrapper = new QueryWrapper<>();
    ...
    private List<StoreMenuVO> tree(List<StoreMenu> menus) {
        ...
        menus.forEach(item -> {
            if (item.getLevel() == 0 && !item.getDeleteFlag()) {
    ...
    private void initChild(StoreMenuVO tree, List<StoreMenu> menus) {
        ...
        menus.stream()
                .filter(item -> (item.getParentId().equals(tree.getId())&&!item.getDeleteFlag()))
                .forEach(child -> {
