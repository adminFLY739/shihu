...
public class GoodsCollectionServiceImpl extends ServiceImpl<GoodsCollectionMapper, GoodsCollection> implements GoodsCollectionService {
    ...
    @Override
    public IPage<GoodsCollectionVO> goodsCollection(String tenantId,PageVO pageVo) {
        QueryWrapper<GoodsCollectionVO> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("gc.member_id", Objects.requireNonNull(UserContext.getCurrentUser()).getId());
        queryWrapper.eq("gs.tenant_id", tenantId);
    ...
    public GoodsCollection addGoodsCollection(String skuId) {
        GoodsCollection goodsCollection = this.getOne(new LambdaUpdateWrapper<GoodsCollection>()
                .eq(GoodsCollection::getMemberId, Objects.requireNonNull(UserContext.getCurrentUser()).getId())
                .eq(GoodsCollection::getSkuId, skuId));
        if (goodsCollection == null) {
            goodsCollection = new GoodsCollection(Objects.requireNonNull(UserContext.getCurrentUser()).getId(), skuId);
