...
public interface StoreLogisticsMapper extends BaseMapper<StoreLogistics> {
     ...
     * @return 物流公司列表
     */
    @Select("SELECT sl.logistics_id,l.name,sl.face_sheet_flag FROM li_logistics l INNER JOIN li_store_logistics sl on sl.logistics_id=l.id WHERE l.disabled = 'OPEN' AND store_id=#{storeId} AND tenant_id=#{tenantId};")
    List<StoreLogisticsVO> getOpenStoreLogistics(String storeId,String tenantId);
     ...
     * @return 物流公司列表
     */
    @Select("SELECT id as logistics_id,name FROM li_logistics WHERE id not in(SELECT logistics_id FROM li_store_logistics WHERE store_id=#{storeId}) AND disabled = 'OPEN' AND tenant_id=#{tenantId}")
    List<StoreLogisticsVO> getCloseStroreLogistics(String storeId,String tenantId);
