...
import cn.lili.modules.goods.entity.vos.GoodsDataVO;
...
public class GoodsServiceImpl extends ServiceImpl<GoodsMapper, Goods> implements GoodsService {
    ...
    public void underStoreGoods(String storeId,String tenantId) {
        //获取商品ID列表
        List<String> list = this.baseMapper.getGoodsIdByStoreId(storeId,tenantId);
        //下架店铺下的商品
    ...
    public IPage<GoodsVO> queryByParams(GoodsSearchParams goodsSearchParams) {
        return baseMapper.queryGoodsList(PageUtil.initPage(goodsSearchParams), goodsSearchParams.queryWrapper());
    }
    ...
    @Override
    public GoodsDataVO getOrderInde(String tenantId) {
        GoodsDataVO goodsDataVO = new GoodsDataVO();
        goodsDataVO.setGoodsNum(goodsStatisticsService.goodsNum(null, null,tenantId));
        goodsDataVO.setGoodsAudit(goodsStatisticsService.goodsNum(null,GoodsAuthEnum.TOBEAUDITED,tenantId));
        goodsDataVO.setGoodsUpper(goodsStatisticsService.goodsNum(GoodsStatusEnum.DOWN,null,tenantId));
        return goodsDataVO;
    }
    ...
    private void checkGoods(Goods goods) {
        ...
        //获取商品系统配置决定是否审核
        Setting setting = settingService.getByIdAndTenantId(SettingEnum.GOODS_SETTING.name(),goods.getTenantId());
