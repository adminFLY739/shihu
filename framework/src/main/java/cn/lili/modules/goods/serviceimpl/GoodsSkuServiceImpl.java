...
public class GoodsSkuServiceImpl extends ServiceImpl<GoodsSkuMapper, GoodsSku> implements GoodsSkuService {
    ...
    public Map<String, Object> getGoodsSkuDetail(String goodsId, String skuId) {
        ...
        if (promotionMap != null && !promotionMap.isEmpty()) {
            promotionMap = promotionMap.entrySet().stream().parallel().filter(i -> {
                ...
                // 过滤活动赠送优惠券和无效时间的活动
                return (jsonObject.get("getType") == null || jsonObject.get("getType", String.class).equals(CouponGetEnum.FREE.name())) &&(jsonObject.get("tenantId")==null || jsonObject.get("tenantId",String.class).equals(goodsSkuDetail.getTenantId())) &&(jsonObject.get("startTime") != null && jsonObject.get("startTime", Date.class).getTime() <= System.currentTimeMillis()) && (jsonObject.get("endTime") == null || jsonObject.get("endTime", Date.class).getTime() >= System.currentTimeMillis());
        ...
        map.put("categoryName", categoryService.getCategoryNameByIds(Arrays.asList(split),goodsSkuDetail.getTenantId()));
        ...
        if (UserContext.getCurrentUser() != null) {
            FootPrint footPrint = new FootPrint(UserContext.getCurrentUser().getId(), goodsIndex.getStoreId(), goodsId, skuId, goodsVO.getTenantId());
    ...
    public List<GoodsSkuVO> getGoodsSkuVOList(List<GoodsSku> list) {
        ...
        for (GoodsSku goodsSku : list) {
            ...
            goodsSkuVO.setShipmentsName(shipmentsService.getById(goodsSkuVO.getSn()).getName());
    ...
    public IPage<GoodsSku> getGoodsSkuByPage(GoodsSearchParams searchParams) {
        return this.baseMapper.queryGoodsSkuList(PageUtil.initPage(searchParams), searchParams.queryWrapper());
    }
