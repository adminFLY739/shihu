...
public class MemberWalletServiceImpl extends ServiceImpl<MemberWalletMapper, MemberWallet> implements MemberWalletService {
    ...
    @Override
    public MemberWalletVO getMemberWallet(String memberId,String tenantId) {
        //构建查询条件
        ...
        if (memberWallet == null) {
            memberWallet = this.save(memberId, memberService.getById(memberId).getUsername(),tenantId);
        }
        //返回查询数据
    ...
    public Boolean increaseWithdrawal(MemberWalletUpdateDTO memberWalletUpdateDTO) {
        ...
        MemberWallet memberWallet = this.checkMemberWallet(memberWalletUpdateDTO.getMemberId(),memberWalletUpdateDTO.getTenantId());
        //余额变动
     ...
     * @param memberId 会员id
     */
    private MemberWallet checkMemberWallet(String memberId,String tenantId) {
        //获取会员预存款信息
        MemberWallet memberWallet = this.getOne(new QueryWrapper<MemberWallet>().eq("member_id", memberId).eq("tenant_id",tenantId), false);
        //如果会员预存款信息不存在则同步重新建立预存款信息
        if (memberWallet == null) {
            Member member = memberService.getById(memberId);
            if (member != null) {
                memberWallet = this.save(memberId, member.getUsername(),tenantId);
            } else {
    ...
    @Override
    public MemberWallet save(String memberId, String memberName,String tenantId) {
        //获取会员预存款信息
        MemberWallet memberWallet = this.getOne(new QueryWrapper<MemberWallet>().eq("member_id", memberId).eq("tenant_id",tenantId));
        if (memberWallet != null) {
            return memberWallet;
        }
        memberWallet = new MemberWallet();
        memberWallet.setMemberId(memberId);
        memberWallet.setTenantId(tenantId);
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public Boolean applyWithdrawal(Double price ,String tenantId) {
        ...
        memberWithdrawApply.setTenantId(tenantId);
        //校验该次提现是否需要审核,如果未进行配置 默认是需要审核
        Setting setting = settingService.getByIdAndTenantId(SettingEnum.WITHDRAWAL_SETTING.name(),"0");
        if (setting != null) {
            //如果不需要审核则审核自动通过
            ...
            if (Boolean.FALSE.equals(withdrawalSetting.getApply())) {
                ...
                MemberWalletVO memberWalletVO = this.getMemberWallet(memberWithdrawApply.getMemberId(),tenantId);
                ...
                if (Boolean.TRUE.equals(result)) {
                    this.reduce(new MemberWalletUpdateDTO(price, authUser.getId(), "余额提现成功", tenantId,DepositServiceTypeEnum.WALLET_WITHDRAWAL.name()));
                }
            } else {
                memberWithdrawalMessage.setStatus(WithdrawStatusEnum.APPLY.name());
                //扣减余额到冻结金额
                this.reduceWithdrawal(new MemberWalletUpdateDTO(price, authUser.getId(), "提现金额已冻结，审核成功后到账",tenantId,DepositServiceTypeEnum.WALLET_WITHDRAWAL.name()));
            }
            //发送余额提现申请消息
