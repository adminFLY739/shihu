...
public class AdminUserServiceImpl extends ServiceImpl<AdminUserMapper, AdminUser> implements AdminUserService {
    ...
    public IPage<AdminUserVO> adminUserPage(Page initPage, QueryWrapper<AdminUser> initWrapper) {
        ...
        adminUserPage.getRecords().forEach(adminUser -> {
            ...
            if (!CharSequenceUtil.isEmpty(adminUser.getTenantId())) {
                try {
                    adminUserVO.setTenantName(
                            tenants.stream().filter
                                            (tenant -> tenant.getId().equals(adminUser.getTenantId()))
                                    .collect(Collectors.toList())
                                    .get(0)
                                    .getName()
                    );
    ...
    public AdminUser findByUsername(String username) {
        ...
        return getOne(new LambdaQueryWrapper<AdminUser>().eq(AdminUser::getUsername, username));
    }
