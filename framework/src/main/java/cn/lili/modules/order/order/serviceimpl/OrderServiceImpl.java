...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, Order> implements OrderService {
    ...
    public void intoDB(TradeDTO tradeDTO) {
        ...
        tradeDTO.getCartList().forEach(item -> {
            ...
            //记录日志
            orderLogs.add(new OrderLog(item.getSn(), Objects.requireNonNull(UserContext.getCurrentUser()).getId(), UserContext.getCurrentUser().getRole().getRole(), UserContext.getCurrentUser().getUsername(), message));
    ...
    public IPage<OrderSimpleVO> queryByParams(OrderSearchParams orderSearchParams) {
        ...
        if(currentUser.getRole().equals(UserEnums.STORE) && currentUser.getClerkId() != null&& shipmentsClerkService.isInShipments(currentUser.getClerkId())) {
           orderSearchParams.setClerkId(currentUser.getClerkId());
        }
    ...
    public Order updateConsignee(String orderSn, MemberAddressDTO memberAddressDTO) {
        ...
        OrderLog orderLog = new OrderLog(orderSn, Objects.requireNonNull(UserContext.getCurrentUser()).getId(), UserContext.getCurrentUser().getRole().getRole(), UserContext.getCurrentUser().getUsername(), message);
        orderLogService.save(orderLog);
    ...
    public Traces getTraces(String orderSn) {
        ...
        Order order = this.getBySn(orderSn);
        //获取踪迹信息
        return logisticsService.getLogisticTrack(order.getLogisticsCode(), order.getLogisticsNo(), order.getConsigneeMobile(),order.getTenantId());
    }
    ...
    public Traces getMapTraces(String orderSn) {
        ...
        //获取踪迹信息
        return logisticsService.getLogisticMapTrack(order.getLogisticsCode(), order.getLogisticsNo(), order.getConsigneeMobile(), from, to,order.getTenantId());
    }
    ...
    public Order take(String verificationCode) {
        String storeId = OperationalJudgment.judgment(Objects.requireNonNull(UserContext.getCurrentUser())).getStoreId();
    ...
    private void checkBatchDeliver(List<OrderBatchDeliverDTO> list) {
        ...
        for (OrderBatchDeliverDTO orderBatchDeliverDTO : list) {
            ...
            Order order = this.getOne(new LambdaQueryWrapper<Order>()
                    .eq(Order::getStoreId, Objects.requireNonNull(UserContext.getCurrentUser()).getStoreId())
    ...
    @Override
    public OrderDataVO getOrderInde(String tenantId) {
        OrderDataVO  orderDataVO = new OrderDataVO ();
        //获取总订单数量
        orderDataVO.setOrderNum(orderStatisticsService.orderNum(null,tenantId));
        orderDataVO.setOrderUndeliverdNum(orderStatisticsService.orderNum(OrderStatusEnum.UNDELIVERED.name(),tenantId));
        orderDataVO.setOrderTakeNum(orderStatisticsService.orderNum(OrderStatusEnum.TAKE.name(),tenantId));
        orderDataVO.setOrderCompletedNum(orderStatisticsService.orderNum(OrderStatusEnum.COMPLETED.name(),tenantId));
        return orderDataVO;
    }
    ...
    private void checkTradeDTO(TradeDTO tradeDTO) {
        //检测是否为拼团订单
        if (CharSequenceUtil.isNotEmpty(tradeDTO.getParentOrderSn())) {
            //判断用户不能参与自己发起的拼团活动
            Order parentOrder = this.getBySn(tradeDTO.getParentOrderSn());
            if (parentOrder.getMemberId().equals(Objects.requireNonNull(UserContext.getCurrentUser()).getId())) {
