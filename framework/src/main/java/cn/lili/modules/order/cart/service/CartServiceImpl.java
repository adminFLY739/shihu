...
public class CartServiceImpl implements CartService {
    ...
    public void add(String skuId, Integer num, String cartType, Boolean cover,String tenantId) {
        ...
        try {
            ...
            if (cartTypeEnum.equals(CartTypeEnum.CART)) {
                ...
                tradeDTO = this.readDTO(cartTypeEnum,dataSku.getTenantId());
    ...
    public void checked(String skuId, boolean checked) {
        ...
        TradeDTO tradeDTO = this.readDTO(CartTypeEnum.CART,goodsSku.getTenantId());
    ...
    @Override
    public void checkedStore(String storeId, boolean checked,String tenantId) {
        TradeDTO tradeDTO = this.readDTO(CartTypeEnum.CART,tenantId);
    ...
    @Override
    public void checkedAll(boolean checked,String tenantID) {
        TradeDTO tradeDTO = this.readDTO(CartTypeEnum.CART,tenantID);
    ...
    @Override
    public void delete(String[] skuIds,String tenantId) {
        TradeDTO tradeDTO = this.readDTO(CartTypeEnum.CART,tenantId);
        List<CartSkuVO> cartSkuVOS = tradeDTO.getSkuList();
    ...
    public void clean(String tenantId) {
        cache.remove(this.getOriginKey(CartTypeEnum.CART,tenantId));
    }
    ...
    @Override
    public TradeDTO getCheckedTradeDTO(CartTypeEnum way,String tenantId) {
        return tradeBuilder.buildChecked(way,tenantId);
    }
     ...
     */
    @Override
    public Long getCanUseCoupon(CartTypeEnum checkedWay,String tenantId) {
        TradeDTO tradeDTO = this.readDTO(checkedWay,tenantId);
        long count = 0L;
    ...
    @Override
    public TradeDTO getAllTradeDTO(String tenantId) {
        return tradeBuilder.buildCart(CartTypeEnum.CART,tenantId);
    }
    ...
    @Override
    public void shippingAddress(String shippingAddressId, String way,String tenantId) {
        ...
        TradeDTO tradeDTO = this.readDTO(cartTypeEnum,tenantId);
    ...
    @Override
    public void shippingSelfAddress(String shopAddressId, String way,String tenantId) {
        //默认购物车
        CartTypeEnum cartTypeEnum = CartTypeEnum.CART;
        if (CharSequenceUtil.isNotEmpty(way)) {
            cartTypeEnum = CartTypeEnum.valueOf(way);
     ...
     */
    @Override
    public void shippingReceipt(ReceiptVO receiptVO, String way,String tenantId) {
        CartTypeEnum cartTypeEnum = CartTypeEnum.CART;
        if (CharSequenceUtil.isNotEmpty(way)) {
            cartTypeEnum = CartTypeEnum.valueOf(way);
        }
     ...
     */
    @Override
    public void shippingMethod(String deliveryMethod, String way,String tenantId) {
        CartTypeEnum cartTypeEnum = CartTypeEnum.CART;
        if (CharSequenceUtil.isNotEmpty(way)) {
            cartTypeEnum = CartTypeEnum.valueOf(way);
        }
     ...
     */
    @Override
    public Long getCartNum(Boolean checked,String tenantId) {
        //构建购物车
        TradeDTO tradeDTO = this.getAllTradeDTO(tenantId);
        //过滤sku列表
    ...
    public void selectCoupon(String couponId, String way, boolean use,String tenantId) {
        AuthUser currentUser = Objects.requireNonNull(UserContext.getCurrentUser());
        //获取购物车，然后重新写入优惠券
    ...
    public Trade createTrade(TradeParams tradeParams) {
        ...
        TradeDTO tradeDTO = this.readDTO(cartTypeEnum,tradeParams.getTenantId());
        //设置基础属性
        tradeDTO.setClientType(tradeParams.getClient());
        tradeDTO.setStoreRemark(tradeParams.getRemark());
        tradeDTO.setTenantId(tradeParams.getTenantId());
        ...
        this.cleanChecked(this.readDTO(cartTypeEnum,trade.getTenantId()));
        return trade;
    }

    @Override
    public List<String> shippingMethodList(String way,String tenantId) {
        List<String> list = new ArrayList<String>();
        list.add(DeliveryMethodEnum.LOGISTICS.name());
        TradeDTO tradeDTO = this.getCheckedTradeDTO(CartTypeEnum.valueOf(way),tenantId);
