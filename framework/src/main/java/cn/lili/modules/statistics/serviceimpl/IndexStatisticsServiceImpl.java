...
public class IndexStatisticsServiceImpl implements IndexStatisticsService {
    ...
    @Override
    public IndexNoticeVO indexNotice(String tenantId) {

        IndexNoticeVO indexNoticeVO = new IndexNoticeVO();
        //商品审核
        indexNoticeVO.setGoods(goodsStatisticsService.goodsNum(null, GoodsAuthEnum.TOBEAUDITED, tenantId));
        //店铺入驻审核
        indexNoticeVO.setStore(storeStatisticsService.auditNum(tenantId));
        //售后申请
        indexNoticeVO.setRefund(afterSaleStatisticsService.applyNum(null,tenantId));
        //投诉审核
        indexNoticeVO.setComplain(orderComplaintStatisticsService.waitComplainNum(tenantId));
        //分销员提现审核
//        indexNoticeVO.setDistributionCash(distributionCashStatisticsService.newDistributionCash());
        ...
        //待处理用户申请
        indexNoticeVO.setMember(memberTenantService.memberNum(MemberStatusEnum.APPLYING,tenantId));
        return indexNoticeVO;
    }

    @Override
    public IndexStatisticsVO indexStatistics(String tenantId) {

        //首页统计内容
        IndexStatisticsVO indexStatisticsVO = new IndexStatisticsVO();

        //获取总订单数量
        indexStatisticsVO.setOrderNum(orderStatisticsService.orderNum(null,tenantId));
        //获取总会员数量
        indexStatisticsVO.setMemberNum(memberStatisticsService.getMemberCount(tenantId));
        //获取总上架商品数量
        indexStatisticsVO.setGoodsNum(goodsStatisticsService.goodsNum(GoodsStatusEnum.UPPER, GoodsAuthEnum.PASS,tenantId));
        //获取总店铺数量
        indexStatisticsVO.setStoreNum(storeStatisticsService.storeNum(tenantId));

        //下单统计
        Map<String, Object> map = storeFlowStatisticsService.getOrderStatisticsPrice(tenantId);
        //今日下单数
        ...
        //今日新增商品数量
        indexStatisticsVO.setTodayGoodsNum(goodsStatisticsService.todayUpperNum(tenantId));
        //今日新增店铺数量
        indexStatisticsVO.setTodayStoreNum(storeStatisticsService.todayStoreNum(tenantId));
        //今日新增评论数量
...
//       三十日访问数UV
    ...
    @Override
    public StoreIndexStatisticsVO storeIndexStatistics(String tenantId) {

        String storeId = Objects.requireNonNull(UserContext.getCurrentUser()).getStoreId();
        StoreIndexStatisticsVO storeIndexStatisticsVO = new StoreIndexStatisticsVO();

        //商品总数量
        storeIndexStatisticsVO.setGoodsNum(goodsStatisticsService.goodsNum(GoodsStatusEnum.UPPER, null,tenantId));
        //订单总数量、订单总金额
        Map<String, Object> map = storeFlowStatisticsService.getOrderStatisticsPrice(tenantId);
        ...
        //待付款订单数量
        storeIndexStatisticsVO.setUnPaidOrder(orderStatisticsService.orderNum(OrderStatusEnum.UNPAID.name(),tenantId));
        //待发货订单数量
        storeIndexStatisticsVO.setUnDeliveredOrder(orderStatisticsService.orderNum(OrderStatusEnum.UNDELIVERED.name(),tenantId));
        //待收货订单数量
        storeIndexStatisticsVO.setDeliveredOrder(orderStatisticsService.orderNum(OrderStatusEnum.DELIVERED.name(),tenantId));
        //待自提数量
        storeIndexStatisticsVO.setSelfPickNum(orderStatisticsService.orderNum(OrderStatusEnum.STAY_PICKED_UP.name(),tenantId));
        //待处理退货数量
        storeIndexStatisticsVO.setReturnGoods(afterSaleStatisticsService.applyNum(AfterSaleTypeEnum.RETURN_GOODS.name(),tenantId));
        //待处理退款数量
        storeIndexStatisticsVO.setReturnMoney(afterSaleStatisticsService.applyNum(AfterSaleTypeEnum.RETURN_MONEY.name(),tenantId));
        //待回复评价数量
        storeIndexStatisticsVO.setMemberEvaluation(memberEvaluationStatisticsService.getWaitReplyNum());
        //待处理投诉数量
        storeIndexStatisticsVO.setComplaint(orderComplaintStatisticsService.waitComplainNum(tenantId));

        //待上架商品数量
        storeIndexStatisticsVO.setWaitUpper(goodsStatisticsService.goodsNum(GoodsStatusEnum.DOWN, null,tenantId));
        //待审核商品数量
        storeIndexStatisticsVO.setWaitAuth(goodsStatisticsService.goodsNum(null, GoodsAuthEnum.TOBEAUDITED,tenantId));
    ...
    public List<StoreStatisticsDataVO> storeStatistics(StatisticsQueryParam statisticsQueryParam) {
        ...
        queryWrapper.groupBy("me.store_id,me.store_name ");
