...
public class StoreFlowStatisticsServiceImpl extends ServiceImpl<StoreFlowStatisticsMapper, StoreFlow> implements StoreFlowStatisticsService {
    ...
    public List<GoodsStatisticsDataVO> getGoodsStatisticsData(GoodsStatisticsQueryParam goodsStatisticsQueryParam, Integer num) {
        //获取查询条件
        QueryWrapper<GoodsStatisticsDataVO> queryWrapper = getQueryWrapper(goodsStatisticsQueryParam);
        //根据商品分组
        queryWrapper.groupBy("goods_id");
        queryWrapper.groupBy("me.goods_name");

        queryWrapper.eq(!StringUtils.isEmpty(goodsStatisticsQueryParam.getStoreId()), "me.store_id", goodsStatisticsQueryParam.getStoreId());
        queryWrapper.eq(CharSequenceUtil.isNotEmpty(goodsStatisticsQueryParam.getTenantId()),"tenant_id",goodsStatisticsQueryParam.getTenantId());
        //查询前X记录
    ...
    @Override
    public Map<String, Object> getOrderStatisticsPrice(String tenantId) {

        QueryWrapper<StoreFlow> queryWrapper = Wrappers.query();
        //支付订单
        ...
        if (authUser.getRole().equals(UserEnums.STORE)) {
            queryWrapper.eq("me.store_id", authUser.getStoreId());
        }
        //大于今天凌晨
        queryWrapper.ge("me.create_time", cn.lili.common.utils.DateUtil.startOfTodDayTime());
        if(CharSequenceUtil.isNotEmpty(tenantId)){
            queryWrapper.eq("tenant_id",tenantId);
        }


        baseMapper.getOrderStatisticsPrice(queryWrapper);

        return baseMapper.getOrderStatisticsPrice(queryWrapper).get(0);
    }
    ...
    private void initOrder(Date[] dates, OrderOverviewVO orderOverviewVO, StatisticsQueryParam statisticsQueryParam) {
        //构建查询条件
        QueryWrapper<StoreFlow> queryWrapper = Wrappers.query();
        queryWrapper.eq(CharSequenceUtil.isNotEmpty(statisticsQueryParam.getTenantId()),"g.tenant_id",statisticsQueryParam.getTenantId());
        //时间区间
        queryWrapper.between("me.create_time", dates[0], dates[1]);
        //如果有店铺id传入，则查询店铺
        if (StringUtils.isNotEmpty(statisticsQueryParam.getStoreId())) {
            queryWrapper.eq("me.store_id", statisticsQueryParam.getStoreId());
        }
        //获取查询结果
        Map<String,Object> order = baseMapper.getOrderStatisticsPrice(queryWrapper).get(0);
        //赋予订单数和流水金额
        ...
        //写入下单人数
        orderOverviewVO.setOrderMemberNum(order != null && order.containsKey("memberNum") ? (Long) order.get("memberNum") : 0L);
    }
    ...
    private void initPayment(Date[] dates, OrderOverviewVO orderOverviewVO, StatisticsQueryParam statisticsQueryParam) {
        ...
        queryWrapper.eq("flow_type", FlowTypeEnum.PAY.name());
        Map<String,Object> payment = baseMapper.getOrderStatisticsPrice(queryWrapper).get(0);

        orderOverviewVO.setPaymentOrderNum(payment != null && payment.containsKey("num") ? (Long) payment.get("num") : 0L);
        orderOverviewVO.setPaymentAmount(payment != null && payment.containsKey("price") ? (Double) payment.get("price") : 0L);
        orderOverviewVO.setPaymentsNum(payment != null && payment.containsKey("memberNum") ? (Long) payment.get("memberNum") : 0L);
...
//        }
    ...
    private QueryWrapper getQueryWrapper(GoodsStatisticsQueryParam goodsStatisticsQueryParam) {
        ...
        queryWrapper.between("me.create_time", date[0], date[1]);
