...
public class OrderStatisticsServiceImpl extends ServiceImpl<OrderStatisticsMapper, Order> implements OrderStatisticsService {
    ...
    @Override
    public long orderNum(String orderStatus , String tenantId) {
        OrderSearchParams orderSearchParams = new OrderSearchParams();
        if(CharSequenceUtil.isNotEmpty(orderStatus)){
            orderSearchParams.setOrderStatus(orderStatus);
        }
        if(CharSequenceUtil.equals(Objects.requireNonNull(UserContext.getCurrentUser()).getRole().name(), UserEnums.STORE.name())){
            orderSearchParams.setStoreId(UserContext.getCurrentUser().getStoreId());
        }
        if(CharSequenceUtil.isNotEmpty(tenantId)){
            orderSearchParams.setTenantId(tenantId);
        }
        return orderService.queryByParams(orderSearchParams).getTotal();
    }
    ...
    public IPage<OrderSimpleVO> getStatistics(StatisticsQueryParam statisticsQueryParam, PageVO pageVO) {
        ...
        queryWrapper.eq(CharSequenceUtil.isNotEmpty(statisticsQueryParam.getTenantId()),"o.tenant_id",statisticsQueryParam.getTenantId());
    ...
    private QueryWrapper<Order> getQueryWrapper(StatisticsQueryParam statisticsQueryParam) {
