...
public class WechatMessageUtil {
    ...
    public void sendWechatWaybill(String sn) {
      try {
        this.wechatMpWaybill(sn);
      } catch (Exception e) {
        log.error("小程序消息订阅异常：", e);
      }
    ...
    private void wechatMpWaybill(String sn) {
      Order order = orderService.getBySn(sn);
      Tenant tenant = tenantAreaService.getById(order.getTenantId());
      //获取用户openid
      Connect connect = connectService.queryConnect(
        ConnectQueryDTO.builder().userId(order.getMemberId()).unionType(ConnectEnum.WECHAT_MP_OPEN_ID.name()).appid(tenant.getAppid()).build()
      );
      if (connect == null) {
        return;
      }
      String token = wechatAccessTokenUtil.cgiAccessToken(ClientTypeEnum.WECHAT_MP,order.getTenantId());

      //发送url
      String url = "https://api.weixin.qq.com/cgi-bin/express/delivery/open_msg/follow_waybill?access_token=" + token;

      Map<String, Object> map = new HashMap<>();
      //用户openid
      map.put("openid", connect.getUnionId());
      //收件人手机号id
      map.put("receiver_phone", order.getConsigneeMobile());
      Logistics logistics = logisticsService.getById(order.getLogisticsCode());
      //运力id
      map.put("delivery_id",logistics.getCode());
      //运单号
      map.put("waybill_id", order.getLogisticsNo());
      //交易单号（微信支付生成的交易单号，一般以420开头）
      map.put("trans_id", order.getReceivableNo());
      List<Map<String, String>> detailList = new ArrayList<>();
      List<OrderItem> orderItems = orderItemService.getByOrderSn(order.getSn());
      orderItems.forEach(orderItem -> {
        Map<String,String> detailMap = new HashMap<>();
        detailMap.put("goods_name",orderItem.getGoodsName());
        detailMap.put("goods_img_url",orderItem.getImage());
        detailList.add(detailMap);
      });
      //模版中所需数据
      Map<String, List<Map<String, String>>> postParams = new HashMap<>();
      postParams.put("detail_list",detailList);
      map.put("goods_info", postParams);
      map.put("order_detail_path", "pages/order/orderDetail?sn=" + order.getSn());
      log.info("参数内容：" + JSONUtil.toJsonStr(map));
      String content = null;
      try {
        content = HttpUtil.post(url, JSONUtil.toJsonStr(map));
      } catch (Exception e) {
        log.error("物流信息发送错误", e);
      }
      JSONObject json = new JSONObject(content);
      log.info("物流信息发送结果：" + content);
  ...
  /**
    ...
    public void wechatMessage(String sn) {
        ...
        //获取token
        String token = wechatAccessTokenUtil.cgiAccessToken(ClientTypeEnum.H5,"0");
