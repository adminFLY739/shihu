...
public class WechatAccessTokenUtil {
     ...
     * @return
     */
    public String cgiAccessToken(ClientTypeEnum clientTypeEnum,String tenantId) {
        //h5 和MP 才有获取token的能力
        if (clientTypeEnum.equals(ClientTypeEnum.H5) || clientTypeEnum.equals(ClientTypeEnum.WECHAT_MP)) {

            //缓存一下token
            String token = cache.getString(CachePrefix.WECHAT_CGI_ACCESS_TOKEN.getPrefix() + clientTypeEnum.name()+tenantId);
            if (token != null) {
                return token;
            }
            //获取微信配置
            Setting setting = settingService.getByIdAndTenantId(SettingEnum.WECHAT_CONNECT.name(),tenantId);
            if (setting == null) {
            ...
            cache.put(CachePrefix.WECHAT_CGI_ACCESS_TOKEN.getPrefix() + clientTypeEnum.name()+tenantId,
    ...
    public String cgiJsApiTicket(ClientTypeEnum clientTypeEnum) {
        ...
        String accessToken = this.cgiAccessToken(clientTypeEnum,"0");
        try {
