...
public class EsGoodsSearchServiceImpl implements EsGoodsSearchService {
    ...
    public SearchPage<EsGoodsIndex> searchGoods(EsGoodsSearchDTO searchDTO, PageVO pageVo) {
        // 如果搜索关键词不为空，则增加热词缓存的分数
        if (CharSequenceUtil.isNotBlank(searchDTO.getKeyword())) {
            cache.incrementScore(CachePrefix.HOT_WORD.getPrefix()+searchDTO.getTenantId(), searchDTO.getKeyword());
        }

        // 创建 NativeSearchQueryBuilder 对象，用于构建搜索查询条件
    ...
    private void commonSearch(BoolQueryBuilder filterBuilder, EsGoodsSearchDTO searchDTO) {
        ...
        //销售模式判定
        if (searchDTO.getSalesModel() != null) {
            filterBuilder.must(QueryBuilders.matchQuery("salesModel",searchDTO.getSalesModel()));
        }
        //租户判定
        if (searchDTO.getTenantId()!=null) {
            List<String> tenantList = Arrays.asList(searchDTO.getTenantId().split(","));
            filterBuilder.must(QueryBuilders.termsQuery("tenantId",tenantList));
        }
        //价格区间判定
