...
public class HotWordsServiceImpl implements HotWordsService {
    ...
    @Override
    public List<String> getHotWords(Integer count,String tenantId) {
        if (count == null) {
            count = 0;
        }
        ...
        count = count - 1;
        Set<ZSetOperations.TypedTuple<Object>> set = cache.reverseRangeWithScores(CachePrefix.HOT_WORD.getPrefix()+tenantId, count);
    ...
    @Override
    public List<HotWordsHistory> getHotWordsVO(Integer count,String tenantId) {
        if (count == null) {
            count = 50;
        }
        ...
        for (ZSetOperations.TypedTuple<Object> defaultTypedTuple : set) {
            try {
                hotWords.add(new HotWordsHistory(defaultTypedTuple.getValue().toString(),
                        defaultTypedTuple.getScore().intValue(),tenantId));
            } catch (Exception e) {
                log.error("读取热词错误", e);
            }
    ...
    public void setHotWords(HotWordsDTO hotWords) {
        cache.incrementScore(CachePrefix.HOT_WORD.getPrefix()+hotWords.getTenantId(), hotWords.getKeywords(), hotWords.getPoint());
    }
     ...
     */
    @Override
    public void deleteHotWords(String keywords,String tenantId) {
        cache.zRemove(CachePrefix.HOT_WORD.getPrefix()+tenantId, keywords);
    }
