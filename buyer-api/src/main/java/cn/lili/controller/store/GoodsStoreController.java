...
public class GoodsStoreController {
     ...
     */
    @Autowired
    private GoodsCampusService goodsCampusService;
    ...
    @ApiOperation(value = "通过id获取子菜单")
    public ResultMessage<List<Category>> getItem(@PathVariable String id) {
        return ResultUtil.data(categoryService.getItem(id));
    }
    ...
    @ApiOperation(value = "分页获取校区")
    @GetMapping(value="/campus")
    public ResultMessage<IPage<GoodsCampus>> getCampusByPage(PageVO pageVO) {
        return ResultUtil.data(goodsCampusService.page(PageUtil.initPage(pageVO)));
    }
