...
public class MemberWalletBuyerController {
    ...
    public ResultMessage<MemberWalletVO> get(String tenantId) {
        AuthUser authUser = UserContext.getCurrentUser();
        if (authUser != null) {
            return ResultUtil.data(memberWalletService.getMemberWallet(authUser.getId(),tenantId));
        }
    ...
    public ResultMessage<Object> updatePassword(@RequestParam @Pattern(regexp = "[a-fA-F0-9]{32}", message = "旧密码格式不正确") String oldPassword,
    ...
    public ResultMessage<Boolean> withdrawal(@Max(value = 9999, message = "充值金额单次最多允许提现9999元") @Min(value = 1, message = "充值金额单次最少提现金额为1元") Double price,String tenantId) {
        return ResultUtil.data(memberWalletService.applyWithdrawal(price,tenantId));
    }
