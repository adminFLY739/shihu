...
public class WalletLogBuyerController {
    ...
    @GetMapping
    public ResultMessage<IPage<WalletLog>> getByPage(PageVO page,String tenantId) {
        //获取当前登录用户
        ...
        IPage<WalletLog> depositLogPage = walletLogService.page(PageUtil.initPage(page),
                new QueryWrapper<WalletLog>().eq("member_id", authUser.getId()).eq("tenant_id",tenantId).orderByDesc("create_time"));
