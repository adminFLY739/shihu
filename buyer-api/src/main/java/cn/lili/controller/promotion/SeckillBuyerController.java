...
public class SeckillBuyerController {
    ...
    @GetMapping
    public ResultMessage<List<SeckillTimelineVO>> getSeckillTime(String tenantId) {
        return ResultUtil.data(seckillApplyService.getSeckillTimeline(tenantId));
    }

    @ApiOperation(value = "获取某个时刻的秒杀活动商品信息")
    @GetMapping("/{timeline}")
    public ResultMessage<List<SeckillGoodsVO>> getSeckillGoods(@PathVariable Integer timeline,String tenantId) {
        return ResultUtil.data(seckillApplyService.getSeckillGoods(timeline,tenantId));
    }
}
