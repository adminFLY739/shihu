...
public class CouponBuyerController {
    ...
    public ResultMessage<List<MemberCoupon>> activity() {
        // 调用 couponActivityService 的 trigger 方法触发自动领取优惠券活动，并返回结果
        return ResultUtil.data(couponActivityService.trigger(
                CouponActivityTrigger.builder()
                        .couponActivityTypeEnum(CouponActivityTypeEnum.AUTO_COUPON)  // 设置优惠券活动类型为自动领取
                        .nickName(Objects.requireNonNull(UserContext.getCurrentUser()).getNickName())  // 获取当前用户的昵称并设置为领取优惠券的昵称
                        .userId(UserContext.getCurrentUser().getId())  // 获取当前用户的ID并设置为领取优惠券的用户ID
                        .build())  // 构建 CouponActivityTrigger 对象
        );
    }
    ...
    public ResultMessage<IPage<CouponVO>> getCouponList(CouponSearchParams queryParam, PageVO page) {
        ...
        IPage<CouponVO> canUseCoupons = couponService.queryCoupon(queryParam, page);
    ...
    public ResultMessage<Object> getMemberCouponsNum(String tenantId) {
        return ResultUtil.data(memberCouponService.getMemberCouponsNum(tenantId));
    }
    ...
    public ResultMessage<Object> receiveCoupon(@NotNull(message = "优惠券ID不能为空") @PathVariable("couponId") String couponId,String tenantId) {
        AuthUser currentUser = Objects.requireNonNull(UserContext.getCurrentUser());
        memberCouponService.receiveBuyerCoupon(couponId, currentUser.getId(), currentUser.getNickName(),tenantId);
