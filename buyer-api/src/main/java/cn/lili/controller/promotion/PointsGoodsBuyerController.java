...
public class PointsGoodsBuyerController {
    ...
    public ResultMessage<IPage<PointsGoodsVO>> getPointsGoodsPage(PointsGoodsSearchParams searchParams, PageVO page) {
        ...
        IPage<PointsGoodsVO> pointsGoodsByPage = pointsGoodsService.queryPointsGoods(PageUtil.initPage(page),searchParams.queryWrapper());
        return ResultUtil.data(pointsGoodsByPage);
    }

    @GetMapping("/category")
    @ApiOperation(value = "获取积分商品分类分页")
    public ResultMessage<IPage<PointsGoodsCategory>> page(String name, PageVO page ,String tenantId) {
        return ResultUtil.data(pointsGoodsCategoryService.getCategoryByPage(name, page , tenantId));
    }
