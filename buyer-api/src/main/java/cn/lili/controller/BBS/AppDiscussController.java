...
/**
 * @author wwx
 * @date 2023/10/16 22:59:01
 */
@Api(tags = "用户端——话题")
@RestController
@RequestMapping("buyer/bbs/discuss")
public class AppDiscussController {
    ...
    @ApiOperation("发布话题")
    public R addDiscuss(@RequestBody AddDiscussForm addDiscussForm){
        AuthUser authUser = UserContext.getCurrentUser();
        if (authUser == null) {
            throw new ServiceException(ResultCode.USER_NOT_LOGIN);
        }
        ValidatorUtils.validateEntity(addDiscussForm);
        Long id = discussService.addDiscuss(addDiscussForm,authUser.getId());
        if (id == 0) {
