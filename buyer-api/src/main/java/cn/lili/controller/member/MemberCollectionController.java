...
public class MemberCollectionController {
    ...
    @GetMapping("/{type}")
    public ResultMessage<Object> goodsList(@PathVariable String type,String tenantId, PageVO page) {
        if (goods.equals(type)) {
            return ResultUtil.data(goodsCollectionService.goodsCollection(tenantId,page));
        }
        return ResultUtil.data(storeCollectionService.storeCollection(tenantId,page));
    }
    ...
    public ResultMessage<Object> addGoodsCollection(@PathVariable String type,
                                                    @NotNull(message = "值不能为空") @PathVariable String id,String tenantId) {
        if (goods.equals(type)) {
            return ResultUtil.data(goodsCollectionService.addGoodsCollection(id));
        }
        return ResultUtil.data(storeCollectionService.addStoreCollection(id,tenantId));
    }
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "type", value = "类型", dataType = "String", paramType = "path", example = "GOODS:商品,STORE:店铺"),
            @ApiImplicitParam(name = "num", value = "值", dataType = "Long", paramType = "path"),
            @ApiImplicitParam(name = "tenantId", value = "tenantId" , required = true , paramType = "query")
    })
    ...
    public ResultMessage<Object> deleteGoodsCollection(@PathVariable String type,
        ...
        return ResultUtil.data(storeCollectionService.deleteStoreCollection(id,tenantId));
    }
