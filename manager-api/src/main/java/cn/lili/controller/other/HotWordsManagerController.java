...
public class HotWordsManagerController {
    ...
    @GetMapping
    public ResultMessage<Object> getHotWords(String tenantId) {
       return ResultUtil.data(hotWordsService.getHotWords(100,tenantId));
    }
    ...
    @DeleteMapping
    public ResultMessage<Object> deleteWords(String words,String tenantId) {
        hotWordsService.deleteHotWords(words,tenantId);
        return ResultUtil.success();
    }

    @ApiOperation(value = "历史热词")
    @GetMapping("/history")
    public ResultMessage<Object> deleteWords(HistorySearchParams historySearchParams,String tenantId) {
        List<HotWordsHistory> hotWordsHistoryList = hotWordsHistoryService.queryByDay(historySearchParams.getDate(),tenantId);
