...
public class SettingManagerController {
    ...
    @PutMapping(value = "/put/{key}/{tenantId}")
    ...
    public ResultMessage saveConfig(@PathVariable String key, @RequestBody String configValue,@PathVariable String tenantId) {
        SettingEnum settingEnum = SettingEnum.valueOf(key);
        //获取系统配置
        Setting setting = settingService.getByIdAndTenantId(settingEnum.name(),tenantId);
        if (setting == null) {
            setting = new Setting();
    ...
    @GetMapping(value = "/get/{key}/{tenantId}")
    @ApiImplicitParam(name = "key", value = "配置key", paramType = "path"
            , allowableValues = "BASE_SETTING,EMAIL_SETTING,GOODS_SETTING,KUAIDI_SETTING,ORDER_SETTING,OSS_SETTING,POINT_SETTING," +
            "WECHAT_PC_CONNECT,WECHAT_WAP_CONNECT,WECHAT_APP_CONNECT,WECHAT_MP_CONNECT," +
            "QQ_WEB_CONNECT,QQ_APP_CONNECT," +
    ...
    public ResultMessage settingGet(@PathVariable String key,@PathVariable String tenantId) {
        return createSetting(key,tenantId);
    }
     ...
     */
    private ResultMessage createSetting(String key,String tenantId) {
        SettingEnum settingEnum = SettingEnum.valueOf(key);
        cache.remove(key+tenantId);
        Setting setting = settingService.getByIdAndTenantId(key,tenantId);
        switch (settingEnum) {
            case BASE_SETTING:
                return setting == null ?
