...
public class AfterSaleReasonManagerController {
    ...
    public ResultMessage<IPage<AfterSaleReason>> getByPage(PageVO page, @RequestParam String serviceType,@RequestParam String tenantId) {
        QueryWrapper<AfterSaleReason> queryWrapper = new QueryWrapper<AfterSaleReason>()
                .eq("service_Type", serviceType);
        if (CharSequenceUtil.isNotEmpty(tenantId)) {
            queryWrapper.eq("tenant_id",tenantId);
        }
        return ResultUtil.data(afterSaleReasonService.page(PageUtil.initPage(page),queryWrapper));
    }
