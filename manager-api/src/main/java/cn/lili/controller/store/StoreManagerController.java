...
public class StoreManagerController {
    ...
    @GetMapping(value = "/get/detail/{storeId}/{tenantId}")
    public ResultMessage<StoreDetailVO> detail(@PathVariable String storeId ,@PathVariable String tenantId) {
        return ResultUtil.data(storeDetailService.getStoreDetailVO(storeId,tenantId));
    }
    ...
    @PutMapping(value = "/audit/{id}/{tenantId}/{passed}")
    public ResultMessage<Object> audit(@PathVariable String id, @PathVariable String tenantId, @PathVariable Integer passed) {
        storeService.audit(id,tenantId, passed);
    ...
    @PutMapping(value = "/disable/{id}/{tenantId}")
    public ResultMessage<Store> disable(@PathVariable String id,@PathVariable String tenantId) {
        storeService.disable(id,tenantId);
        return ResultUtil.success();
    }

    @ApiOperation(value = "开启店铺")
    @ApiImplicitParam(name = "id", value = "店铺id", required = true, dataType = "String", paramType = "path")
    @PutMapping(value = "/enable/{id}/{tenantId}")
    public ResultMessage<Store> enable(@PathVariable String id,@PathVariable String tenantId) {
        storeService.enable(id,tenantId);
        return ResultUtil.success();
    }
    ...
    public ResultMessage<Object> getByMemberId(@Valid @PathVariable String memberId) {
        List<Store> storeList = storeService.list(new QueryWrapper<Store>().eq("member_id", memberId));
        if (storeList.size() > 0) {
            return ResultUtil.data(storeList.get(0));
        }
