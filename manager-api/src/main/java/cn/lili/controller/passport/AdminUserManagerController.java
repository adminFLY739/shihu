...
public class AdminUserManagerController {
    ...
    public ResultMessage<Token> login(@NotNull(message = "用户名不能为空") @RequestParam String username,
        ...
        if (codeImageService.check(uuid, code)) {
    ...
    public ResultMessage<AdminUserVO> getUserInfo() {
        ...
        if (tokenUser != null) {
            ...
            if(CharSequenceUtil.isNotEmpty(adminUser.getTenantId())) {
                adminUserVO.setTenantName(tenantAreaService.getById(adminUser.getTenantId()).getName());
            }
