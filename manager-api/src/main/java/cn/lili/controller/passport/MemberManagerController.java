...
public class MemberManagerController {
    ...
    @PutMapping
    public ResultMessage<Member> update(@Valid ManagerMemberEditDTO managerMemberEditDTO, @RequestParam(required = false) List<String> tenantIds) {
        return ResultUtil.data(memberService.updateMember(managerMemberEditDTO,tenantIds));
    }
    ...
    @DemoSite
    @PreventDuplicateSubmissions
    @ApiOperation(value = "审核通过接口")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "memberIds", value = "会员ID", required = true, dataType = "String", allowMultiple = true, paramType = "query"),
            @ApiImplicitParam(name = "disabled", required = true, dataType = "boolean", paramType = "query")
    })
    @PutMapping("/updateHZNUSHMemberStatus")
    public ResultMessage<Object> updateHZNUSHMemberStatus(@RequestParam List<String> memberIds, @RequestParam Boolean disabled) {
        memberService.updateHZNUSHMemberStatus(memberIds, disabled);
        return ResultUtil.success();
    }
    ...
    @ApiImplicitParam(name = "ids", value = "用户ID", required = true, allowMultiple = true, dataType = "String", paramType = "path")
    @DeleteMapping(value = "/delByIds/{ids}")
    public ResultMessage<Object> delAllByIds(@NotNull(message = "会员ID不能为空") @PathVariable("ids") List<String> ids) {
