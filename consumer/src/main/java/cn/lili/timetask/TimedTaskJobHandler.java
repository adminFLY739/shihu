...
public class TimedTaskJobHandler {
    ...
    @Autowired(required = false)
    private List<EveryDayZeroAMExecute> everyDayZeroAMExecutes;
     ...
     * @throws Exception
     */
    @XxlJob("everyDayZeroAMExecuteJobHandler")
    public ReturnT<String> everyDayZeroAMExecuteJobHandler(String param) {

        log.info("重置每日任务执行");
        if (everyDayZeroAMExecutes == null || everyDayZeroAMExecutes.size() == 0) {
            return ReturnT.SUCCESS;
        }

        for (int i = 0; i < everyDayZeroAMExecutes.size(); i++) {
            try {
                everyDayZeroAMExecutes.get(i).execute();
            } catch (Exception e) {
                log.error("重置每日任务异常", e);
            }
        }
        return ReturnT.SUCCESS;
    }
