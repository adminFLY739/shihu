...
public class HotWordsEveryDayTaskExecute implements EveryDayExecute {
    ...
    public void execute() {
        ...
        tenantList.forEach(tenant -> {
          //获取大于0分的热词
          Set<DefaultTypedTuple> tuples = cache.zRangeByScore(CachePrefix.HOT_WORD.getPrefix()+tenant.getId(), 1, Integer.MAX_VALUE);
          //如果任务不为空
          if (!CollectionUtils.isEmpty(tuples)) {
            ...
            for (DefaultTypedTuple tuple : tuples) {
              ...
              hotWordsHistories.add(new HotWordsHistory(keywords,tenant.getId(),score.intValue(), calendar.getTime()));
            }

            hotWordsHistoryService.saveBatch(hotWordsHistories);
          }
          //移除昨日的热搜词
          cache.remove(CachePrefix.HOT_WORD.getPrefix()+tenant.getId());

          //设置今日默认热词
          Setting setting = settingService.getByIdAndTenantId(SettingEnum.HOT_WORDS.name(),tenant.getId());
          if (setting == null) {
            return;
          }
          ...
          if (hotWordsSettingItems != null && !hotWordsSettingItems.isEmpty()) {
            for (HotWordsSettingItem hotWordsSettingItem : hotWordsSettingItems) {
              cache.zAdd(CachePrefix.HOT_WORD.getPrefix()+tenant.getId(), hotWordsSettingItem.getScore(), hotWordsSettingItem.getKeywords());
            }
          }
        });
