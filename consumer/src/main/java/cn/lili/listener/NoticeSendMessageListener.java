...
public class NoticeSendMessageListener implements RocketMQListener<MessageExt> {
    ...
    private void saveStoreMessage(Message message) {
        ...
        if ("ALL".equals(message.getMessageRange())) {
            ...
            storeSearchParams.setTenantId(message.getTenantId());
            storeSearchParams.setStoreDisable("OPEN");
            PageVO pageVO= new PageVO();
            pageVO.setPageSize(100);
            List<StoreVO> storeList = storeService.findByConditionPage(storeSearchParams,pageVO).getRecords();
    ...
    private void saveMemberMessage(Message message) {
        ...
        if ("ALL".equals(message.getMessageRange())) {
            ...
            memberSearchVO.setTenantId(message.getTenantId());
